...
class DateConv: 
    ...
    def JulianDays(self,date): 
        ...
        J0 = 367*date[2]-(7*(date[2]+((date[1]+9)/12)//1)/4)//1+(275*date[1]/9)//1+date[0]+1721013.5
        return J0
...
class EphemerisJ2000:
    """
    [a (m) ,e, i ,Omega (longitude of ascending node), varpi (longitude of perihelium), L (mean longitude)]
    m, deg, deg/Cy
    """
    def __init__(self):
...
#         self.MMars0 = np.array([0.00000097*1.49597871e11, 0.00009149, -0.00724757    , -0.26852431, 0.45223625 ,19140.29934243])
        ...
        self.Mercury0 = np.array([0.38709927*self.AU , 0.20563593, 7.00497902, 252.25032350, 77.45779628, 48.33076593])
        self.MMercury0 = np.array([0.00000037*self.AU , 0.00001906, -0.00594749, 149472.67411175, 0.16047689, -0.12534081])
        self.Venus0     = np.array([0.72333566*self.AU , 0.00677672, 3.39467605, 181.97909950, 131.60246718, 76.67984255])
        self.VVenus0    = np.array([0.00000390*self.AU , -0.00004107, -0.00078890, 58517.81538729, 0.00268329, -0.27769418])
        self.Earth0   = np.array([1.00000261*self.AU , 0.01671123, -0.00001531, 100.46457166, 102.93768193, 0.0])
        self.EEarth0    = np.array([0.00000562*self.AU , -0.00004392, -0.01294668, 35999.37244981, 0.32327364, 0.0])
        self.Mars0 = np.array([1.52371034*self.AU , 0.09339410, 1.84969142, -4.55343205, -23.94362959, 49.55953891])
        self.MMars0    = np.array([0.00001847*self.AU , 0.00007882, -0.00813131, 19140.30268499, 0.44441088, -0.29257343])
        self.Jupiter0   = np.array([5.20288700*self.AU , 0.04838624, 1.30439695, 34.39644051, 14.72847983, 100.47390909])
        self.JJupiter0 = np.array([-0.00011607*self.AU , -0.00013253, -0.00183714, 3034.74612775, 0.21252668, 0.20469106])
        self.Saturn0 = np.array([9.53667594*self.AU , 0.05386179, 2.48599187, 49.95424423, 92.59887831, 113.66242448])
        self.SSaturn0  = np.array([-0.00125060*self.AU , -0.00050991, 0.00193609, 1222.49362201, -0.41897216, -0.28867794])
        self.Uranus0   = np.array([19.18916464*self.AU , 0.04725744, 0.77263783, 313.23810451, 170.95427630, 74.01692503])
        self.UUranus0  = np.array([-0.00196176*self.AU , -0.00004397, -0.00242939, 428.48202785, 0.40805281, 0.04240589])
        self.Neptune0 = np.array([30.06992276*self.AU , 0.00859048, 1.77004347, -55.12002969, 44.96476227, 131.78422574])
        self.NNeptune0  = np.array([0.00026291*self.AU , 0.00005105, 0.00035372, 218.45945325, -0.32241464, -0.00508664])
        self.Pluto0 = np.array([39.48211675*self.AU, 0.24882730, 17.14001206, 238.92903833, 224.06891629, 110.30393684])
        self.PPluto0 = np.array([-0.00031596*self.AU , 0.00005170, 0.00004818, 145.20780515, -0.04062942, -0.01183482])
...
class EphemerisFile:
    ...
    def __init__(self):
        ...
        self.ephem_E = np.genfromtxt('AstroLibraries_0/PlanetaryEphem/earthState.txt', delimiter = None)
        self.ephem_M = np.genfromtxt('AstroLibraries_0/PlanetaryEphem/marsState.txt', delimiter = None)
        self.ephem_V = np.genfromtxt('AstroLibraries_0/PlanetaryEphem/venusState.txt', delimiter = None)
        self.ephem_J = np.genfromtxt('AstroLibraries_0/PlanetaryEphem/jupiterState.txt', delimiter = None)
