...
def EvolAlgorithm_integerinput(f, bounds, *args, **kwargs):
    ...
    pop_0 = np.zeros([ind, len(bounds)+1])
    for i in range(len(bounds)):
        pop_0[:,i+1] = np.random.uniform(low = bounds[i][0], high = bounds[i][1], size = ind)
...
###############################################
# MONOTONIC BASIN HOPPING batch evaluation of functions and/or local optimization (for parallel computing)
###############################################
def seqEval(f, x):
    """
